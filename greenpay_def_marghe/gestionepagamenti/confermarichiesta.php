...
<div class="container-fluid">
	<div class="row justify-content-center">
		<div class="col-lg-6 my-col5">
			...
			<br>
			<?php
				$codrichiesta = $_GET['cod'];
				echo "<form  method='post' action=\"main.php?codrichiesta=".$codrichiesta."\">";
			?>
			  ...
			  <?php
			   ...
			   $dbname="heroku_16f3a281c2f1459";//nome database
				...
				$sql = "SELECT * FROM richieste join email on mittente=ref_utente WHERE cod_richiesta='".$cod."' AND principale='Principale'";
				$result = $conn->query($sql);

				if ($result->num_rows > 0) {
					$row = $result->fetch_assoc();

					echo "<input type='text' class='form-control' value='".$row['email']."' name='destinatario' id='destinatario' readonly>
							</div>
							<div class='form-group'>
						<label for='importo'>Importo:</label>
						<input type='text' class='form-control' id='importo' name='importo' value='".$row["cifra"]."' readonly>";
				}
	...
	// nome di host
			...
			<option value='gp'>Conto GreenPay</option>;
			  ...
			  <button type="submit" name="submit6" class="btn btn-success">Avanti</button>
			</form>
		</div>
		</div>
	</div>
