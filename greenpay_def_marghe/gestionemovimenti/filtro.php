    ...
    <input type="submit" name="show_dowpdown_value" value="Go!"   class="btn btn-success "/>
		 ...
		 <div class="row justify-content-center">
			<div class="col-lg-8 my-col">
				<h5><b>Cronologia:</b></br></br>
					<center>
					<?php  $tipo=$_POST['tipo']; 
                           $metodo=$_POST['metodo']; 
                           $mese=$_POST['mese']; 
                           $ogg->stampaTipologia($tipo,$mese,$metodo);?></center></h5>
				<br>
...
<?php
 ...
 if ( isset($_POST['show_dowpdown_value'])) {
    ...
    $tipo=$_POST['tipo']; // questo stamperà il valore se il downbox è in uscita
     // $ogg->visualizzaFiltro($tipo);
        $metodo=$_POST['metodo']; // questo stamperà il valore se il downbox è in uscita
        $mese=$_POST['mese']; // questo stamperà il valore se il downbox è in uscita
      $ogg->visualizzaFiltro_v2($tipo,$mese,$metodo);
